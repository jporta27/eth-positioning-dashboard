...
async def fetch_defillama_reserves(client: httpx.AsyncClient) -> Optional[dict]:
    ...
    async def _fetch_one(name: str, slug: str) -> Optional[dict]:
        try:
            ...
            return {"name": name, "ethReserve": round(current_eth, 2), "daily": daily}
        except Exception as e:
    ...
    logger.info(f"DefiLlama reserves ok: {total_eth:,.0f} ETH across {len(exchange_results)} exchanges, history: {list(history.keys())}")
    return result
